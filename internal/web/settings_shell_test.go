...
// The issue-#163 acceptance: Settings, Logs, and Status & backups render as one
// shell with sub-navigation — each page carries the shared h1 + the boosted
...
func TestSettingsShellSubNav(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.route, func(t *testing.T) {
			...
			// All three sections stay reachable from every tab.
			for _, href := range []string{`href="/settings"`, `href="/logs"`, `href="/status"`} {
