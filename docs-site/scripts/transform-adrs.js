...
function fixCrossSectionPaths(content) {
  content = content.replace(/\]\(\.\.\/openspec\/specs\//g, '](../specs/');
  // Repo-root and docs/design/* targets are not part of the docs site;
  // point them at the repo on GitHub. Runs BEFORE fixMarkdownLinks so
  // the .md extension survives into the blob URL.
  content = content.replace(/\]\(\.\.\/\.\.\//g, '](https://github.com/joestump/msgbrowse/blob/main/');
  content = content.replace(/\]\(\.\.\/design\//g, '](https://github.com/joestump/msgbrowse/blob/main/docs/design/');
  return content;
}
...
function transformAdr(srcPath, destPath, fileName) {
  ...
  // fixCrossSectionPaths runs first: it rewrites out-of-site targets to
  // GitHub blob URLs, which must keep their .md extension (fixMarkdownLinks
  // skips http(s) links but would strip .md from the relative form).
  let escapedContent = fixCrossSectionPaths(contentWithoutFrontmatter);
  escapedContent = fixMarkdownLinks(escapedContent);
